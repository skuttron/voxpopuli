...
// FIX 4: null-guard so a missing #newsFeed element never throws and kills other JS
...
// FIX 3: switchTab now forces display style in addition to CSS class toggling.
// CSS-class-only approach fails when initial display isn't set at render time.
...
// FIX 5: null-guard inside news setInterval so it never throws when logged out
if($('newsFeed')){{loadNewsFeed();setInterval(()=>{{if($('newsFeed'))loadNewsFeed();}},300000);}}
// FIX 6: Force correct tab display states on page load
    ...
    with db() as con:
        ...
        for gid in [r[0] for r in fetchall(con,"SELECT id FROM groups")]:
            if not fetchone(con,"SELECT 1 FROM group_banned WHERE group_id=%s AND username=%s",(gid,u)):
                execute(con,"INSERT INTO group_members(group_id,username) VALUES(%s,%s) ON CONFLICT DO NOTHING",(gid,u))
    # FIX 2: Default theme to 'green' if DB column is NULL
