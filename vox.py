...
def is_admin(u=None):
    ...
    if u == ADMIN_USER:
        return True
    with db() as con:
        row = fetchone(con, "SELECT is_admin FROM users WHERE username=%s", (u,))
...
def send_push(username, title, body, tag="vox"):
    try:
        ...
        with db() as con:
            subs = fetchall(con, "SELECT endpoint,p256dh,auth FROM push_subscriptions WHERE username=%s", (username,))
        for endpoint, p256dh, auth in subs:
            try:
                ...
                if hasattr(ex, 'response') and ex.response and ex.response.status_code in (404, 410):
                    with db() as con2:
                        execute(con2, "DELETE FROM push_subscriptions WHERE endpoint=%s", (endpoint,))
    except Exception:
        pass
...
def theme_css(t):
    ...
    return (
        ...
        ".tab:last-child{border-right:0}.tab.active{background:var(--p);color:#000}"
        ...
        ".hero-btn{font-size:10px;padding:5px 8px;letter-spacing:0;border-radius:7px}"
        ".menu-trigger{font-size:10px;padding:5px 8px}"
...
def pw_field(fid, ph, ac="current-password"):
    return (f'<div class="field-wrap">'
            f'<input class="field" id="{fid}" placeholder="{ph}" type="password" autocomplete="{ac}">'
            f'<button class="eye-btn" type="button" onclick="togglePw(\'{fid}\',this)">&#128065;</button>'
            f'</div>')
...
def cyber_box(title, body, *, title_right="", extra_header="", footer="", radius="var(--r)", mb="24px", max_h=None, border_top=True, body_style=""):
    ...
    return (
        f'<div class="command-wrapper" style="width:100%;margin-bottom:{mb};box-sizing:border-box;">'
...
def shell(content, user=None, theme="green", unread=0):
    ...
    if user:
        at_badge = (' <span style="font-size:9px;opacity:.8;margin-left:5px;letter-spacing:1px;vertical-align:middle;">&#9733; ADMIN</span>'
                    if admin else '')
        menu_html = (
            f'<div class="menu-wrap">'
            f'<div class="menu-trigger" onclick="event.stopPropagation();document.getElementById(\'accountMenu\').classList.toggle(\'open\')" '
            f'style="display:flex;align-items:center;gap:8px;padding:8px 16px;border-radius:8px;">'
            f'<span style="font-size:16px;">&#9776;</span>'
            f'<span style="border-left:1px solid var(--p);opacity:.4;height:16px;"></span>'
            f'<span style="font-size:12px;letter-spacing:1px;">{user}</span>{at_badge}'
            f'<span style="font-size:10px;opacity:.6;">&#9663;</span></div>'
            f'<div class="dropdown-menu" id="accountMenu">'
            f'<div class="dropdown-item" style="opacity:.5;font-size:10px;cursor:default;pointer-events:none;padding:8px 16px;">&#9658; {user.upper()} [{t["name"]}]</div>'
            f'<div class="dropdown-divider"></div>'
            f'<a class="dropdown-item" onclick="openModal(\'settingsModal\')"><i class="fas fa-cog"></i> SETTINGS</a>'
            f'<a class="dropdown-item" onclick="enableNotifications()" id="notifMenuItem"><i class="fas fa-bell"></i> ENABLE NOTIFICATIONS</a>'
            f'<a class="dropdown-item" href="/logout"><i class="fas fa-sign-out-alt"></i> LOGOUT</a>'
            f'</div></div>'
        )
    ...
    admin_panel = (
        ...
        '</div>'
        '<div id="adminLookupBar" style="display:none;margin-bottom:8px;"><div style="display:flex;gap:6px;">'
...
<div class="scanline-a"></div>
<div class="scanline-b"></div>
<div class="scanline-c"></div>
...
<div class="page-content" style="width:100%;max-width:960px;margin:0 auto;padding:0 12px 40px;box-sizing:border-box;">
  {content}
</div>
...
<div class="modal-overlay" id="loginModal">
  <div class="modal-box">
    ...
    <div style="margin-top:14px;font-size:11px;opacity:.6;">
      FORGOT YOUR PASSWORD?
      <span style="text-decoration:underline;cursor:pointer;color:var(--p);" onclick="closeModal('loginModal');openModal('resetModal')">REQUEST A RESET</span>
    </div>
  </div>
</div>
...
<div class="modal-overlay" id="registerModal">
  <div class="modal-box">
    ...
    <div class="theme-grid">{theme_btns("setRegTheme")}</div>
    <br>
...
<div class="modal-overlay" id="settingsModal">
  <div class="modal-box" style="max-width:660px;width:96%;">
    ...
    <div id="stContentTheme" class="st-tab-content" style="display:block;">
      <div class="section-label">CHANGE THEME:</div>
      <div class="theme-grid">{theme_btns("changeTheme")}</div>
    </div>
    <div id="stContentPw" class="st-tab-content" style="display:none;">
      <div class="section-label">CHANGE PASSWORD:</div>
      <div id="pwErr" class="error-msg"></div>
      <div id="pwOk" class="success-msg"></div>
      {pw_field("pwCurrent","CURRENT PASSWORD")}
      {pw_field("pwNew","NEW PASSWORD (MIN 6)","new-password")}
      {pw_field("pwNew2","CONFIRM NEW PASSWORD","new-password")}
      <br>
...
async function doLogin(){{
  ...
  else{{errEl.textContent='ERROR: '+d.error;}}
}}

async function doRegister(){{
  const p=$('regPass').value,p2=$('regPass2').value,dob=$('regDob').value,errEl=$('regErr');
  errEl.textContent='';
  if(!dob){{errEl.textContent='DATE OF BIRTH REQUIRED';return;}}
  if((Date.now()-new Date(dob))/31557600000<18){{errEl.textContent='YOU MUST BE 18 OR OLDER TO JOIN';return;}}
...
async function doResetRequest(){{
  const u=$('resetUser').value.trim(),errEl=$('resetErr'),okEl=$('resetOk');
  errEl.textContent='';okEl.textContent='';
  if(!u){{errEl.textContent='USERNAME REQUIRED';return;}}
  const d=await api('/api/reset/request',{{username:u}});
  if(d.ok){{okEl.textContent='REQUEST SENT — AN ADMIN WILL SET A TEMP PASSWORD FOR YOU.';}}
  else{{errEl.textContent='ERROR: '+d.error;}}
}}
...
const msgRow=(m,fn)=>`<div style="padding:6px 10px 6px 20px;border-top:1px solid var(--p10);display:flex;justify-content:space-between;align-items:flex-start;gap:6px;">
  ...
  <button class="btn-action" style="padding:2px 6px;font-size:10px;margin:0;border-color:#f44;color:#f44;flex-shrink:0;" onclick="${{fn}}(${{m.id}})">&#128465;</button>
</div>`;
...
async function adminRemoveUser(u){{
  if(!confirm('REMOVE: '+u+'?'))return;
  const d=await api('/api/admin/remove-user',{{username:u}});
  d.ok?adminShowUsers():alert('ERROR: '+d.error);
}}
...
async function adminShowDMs(){{
  ...
  d.messages.forEach(m=>{{
    ...
    if(!convos[k])convos[k]={{users:[m.sender,m.recipient].sort(),messages:[]}};
    convos[k].messages.push(m);
  }});
  ...
  Object.values(convos).forEach(c=>{{
    const[u1,u2]=c.users;
    html+=`<div style="border-bottom:2px solid var(--p30);">
      <div style="padding:8px 10px;background:var(--p10);display:flex;justify-content:space-between;align-items:center;">
        <span style="font-size:11px;">${{u1}} &#8596; ${{u2}} (${{c.messages.length}})</span>
        <button class="btn-action" style="padding:3px 8px;font-size:10px;margin:0;border-color:#f44;color:#f44;" onclick="adminDeleteConvo('${{u1}}','${{u2}}')">&#128465; ALL</button>
      </div>`;
    c.messages.forEach(m=>{{html+=msgRow(m,'adminDeleteDM');}});
    html+='</div>';
  }});
...
async function adminDeleteConvo(u1,u2){{
  if(!confirm('DELETE CHAT: '+u1+' & '+u2+'?'))return;
  const d=await api('/api/admin/delete-convo',{{user1:u1,user2:u2}});
  d.ok?adminShowDMs():alert('ERROR: '+d.error);
}}
...
async function adminShowGroups(){{
  ...
  const ch={{}};
  d.messages.forEach(m=>{{
    if(!ch[m.group_id])ch[m.group_id]={{id:m.group_id,name:m.group,messages:[],locked:false}};
    ch[m.group_id].messages.push(m);
  }});
  if(dg.ok)dg.groups.forEach(g=>{{
    if(!ch[g.id])ch[g.id]={{id:g.id,name:g.name,messages:[],locked:g.locked}};
    else ch[g.id].locked=g.locked;
  }});
  ...
  Object.values(ch).forEach(c=>{{
    ...
    html+=`<div style="border-bottom:2px solid var(--p30);">
      <div style="padding:8px 10px;background:var(--p10);display:flex;justify-content:space-between;align-items:center;gap:4px;flex-wrap:wrap;">
    ...
    c.messages.forEach(m=>{{html+=msgRow(m,'adminDeleteGroupMsg');}});
    html+='</div>';
  }});
  adminBox().innerHTML=html;
}}

async function adminLockChannel(gid,lock){{
  const d=await api('/api/admin/lock-channel',{{group_id:gid,lock}});
  d.ok?adminShowGroups():alert('ERROR: '+d.error);
}}
async function adminDeleteGroupMsg(id){{await api('/api/admin/delete-group-msg',{{id}});adminShowGroups();}}
async function adminDeleteChannel(gid,gname){{
  if(!confirm('DELETE #'+gname+'?'))return;
  const d=await api('/api/admin/delete-channel',{{group_id:gid}});
  d.ok?adminShowGroups():alert('ERROR: '+d.error);
}}
...
function adminShowLookup(){{
  ...
  $('adminLookupInput').value='';
  $('adminLookupSuggest').style.display='none';
  $('adminLookupInput').focus();
}}
...
async function adminLookupSuggest(){{
  ...
  box.innerHTML=d.users.map(u=>`<div style="padding:10px 14px;cursor:pointer;border-bottom:1px solid var(--p10);" 
    onmouseover="this.style.background='var(--p10)'" onmouseout="this.style.background=''"
...
async function adminLookupRun(){{
  const username=$('adminLookupInput').value.trim();
  if(!username)return;
  $('adminLookupSuggest').style.display='none';
  adminBox().innerHTML='<div style="padding:10px;opacity:.4;text-align:center;">LOADING...</div>';
  ...
  if(!d.conversations.length){{
    adminBox().innerHTML=`<div style="padding:12px;opacity:.4;text-align:center;">NO DM HISTORY FOR ${{username.toUpperCase()}}</div>`;
    return;
  }}
  ...
  d.conversations.forEach(conv=>{{
    html+=`<div style="border-bottom:2px solid var(--p30);">
      <div style="padding:8px 10px;background:var(--p10);display:flex;justify-content:space-between;align-items:center;">
        ...
        <button class="btn-action" style="padding:3px 8px;font-size:10px;margin:0;border-color:#f44;color:#f44;" onclick="adminDeleteConvo('${{username}}','${{conv.partner}}')">&#128465; ALL</button>
      </div>`;
    conv.messages.forEach(m=>{{
      ...
      html+=`<div style="padding:6px 10px 6px ${{mine?'30px':'10px'}};border-top:1px solid var(--p10);display:flex;justify-content:space-between;align-items:flex-start;gap:6px;">
        ...
        <button class="btn-action" style="padding:2px 6px;font-size:10px;margin:0;border-color:#f44;color:#f44;flex-shrink:0;" onclick="adminDeleteDM(${{m.id}});adminLookupRun();">&#128465;</button>
      </div>`;
    }});
    html+='</div>';
  }});
...
async function adminShowTraffic(){{
  ...
  let html=`<div style="padding:8px 10px;background:var(--p10);border-bottom:1px solid var(--p30);display:flex;justify-content:space-between;font-size:11px;">
    <span>&#128200; SITE TRAFFIC</span><span>TODAY: <b>${{d.today}}</b> &nbsp;|&nbsp; ALL TIME: <b>${{d.total}}</b></span></div>`;
  d.days.forEach(r=>{{
    ...
    html+=`<div style="padding:6px 10px;border-bottom:1px solid var(--p10);display:flex;align-items:center;gap:8px;font-size:11px;">
      ...
      <div style="flex:1;background:var(--p10);border-radius:4px;height:14px;overflow:hidden;">
        <div style="width:${{pct}}%;height:100%;background:var(--p);box-shadow:0 0 8px var(--p);border-radius:4px;transition:.3s;"></div>
      </div>
...
async function loadNewsFeed(){{
  try{{
    ...
    const feed=$('newsFeed'),status=$('newsFeedStatus');
    if(!feed)return;
    if(!d.ok||!d.items||!d.items.length){{
      feed.innerHTML='<div style="padding:12px;opacity:.4;text-align:center;font-size:11px;">NO FEED AVAILABLE</div>';
      return;
    }}
    ...
    const srcColors={{'SURVIVAL':'#00ff00','OFF-GRID':'#00ff00','COMMUNITY':'#fb0'}};
    function getPriority(item){{
      ...
      return srcColors[item.cat]||'var(--p)';
    }}
    const priorityLabel={{'#f44':'&#9888; ALERT','#fb0':'&#9650; WATCH','#00ff00':'&#9679; CLEAR'}};
    feed.innerHTML=d.items.map(item=>{{
      const c=getPriority(item),pl=priorityLabel[c]||'';
      const ts=formatNewsTime(item.pub_date);
      return `<div style="padding:10px 14px;border-bottom:1px solid var(--p10);border-left:3px solid ${{c}};display:flex;flex-direction:column;gap:4px;">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
          ...
          ${{ts?`<span style="font-size:9px;opacity:.45;flex-shrink:0;">${{ts}}</span>`:''}}
...
async function loadPosts(){{
  ...
  if(!d.ok||!d.posts||!d.posts.length){{
    feed.innerHTML='<div style="padding:16px;opacity:.4;text-align:center;font-size:11px;">NO POSTS YET - BE THE FIRST!</div>';
    return;
  }}
  const EMOJI_LIST=[['&#128077;','&#128078;','&#10084;&#65039;','&#128514;','&#128558;','&#128545;','&#128293;'],['like','dislike','love','lol','wow','angry','fire']];
  const myName=d.me||'';
  feed.innerHTML=d.posts.map(p=>{{
    const mine=p.username===myName;
    const rxBar=EMOJI_LIST[0].map((e,i)=>{{
      const r=(p.reactions&&p.reactions[EMOJI_LIST[1][i]])||{{count:0,mine:false}};
      return `<button onclick="reactPost(${{p.id}},'${{EMOJI_LIST[1][i]}}')" style="background:${{r.mine?'var(--p)':'var(--p10)'}};color:${{r.mine?'#000':'var(--p)'}};border:1px solid ${{r.mine?'var(--p)':'var(--p30)'}};border-radius:20px;padding:2px 8px;font-size:12px;cursor:pointer;margin:2px;transition:.15s;">${{e}}${{r.count?' <b>'+r.count+'</b>':''}}</button>`;
    }}).join('');
...
async function submitPost(){{
  const content=$('postContent').value.trim(),errEl=$('postErr');
  errEl.textContent='';
...
function enableNotifications(){{
  ...
  Notification.requestPermission().then(p=>{{
    if(p==='granted'){{_notifPermission=true;setupPushSubscription();const m=$('notifMenuItem');if(m)m.style.display='none';}}
...
function requestNotifPermission(){{
  ...
  if(Notification.permission!=='denied')
    Notification.requestPermission().then(p=>{{_notifPermission=p==='granted';if(_notifPermission)setupPushSubscription();}});
}}
...
async function setupPushSubscription(){{
  try{{
    ...
    if(existing){{await api('/api/push/subscribe',existing.toJSON());return;}}
    const kd=await api('/api/push/vapid-public-key');
    if(!kd.ok||!kd.key)return;
    const sub=await reg.pushManager.subscribe({{userVisibleOnly:true,applicationServerKey:urlBase64ToUint8Array(kd.key)}});
  ...
  if(_notifPermission&&Notification.permission==='granted'){{
    try{{
      ...
      n.onclick=()=>{{window.focus();n.close();
        ...
        else if(action==='board')switchTab('board');
      }};
    }}catch(e){{}}
  }}
}}
...
async function checkNotifications(){{
  try{{
    ...
    if(!_notifReady){{
      _prevNotif={{dm:d.dm,group:d.group,private:d.private,posts:d.posts,groups:d.groups||{{}},private_rooms:d.private_rooms||({{}})}};
      _notifReady=true;
    ...
    if(d.posts>0&&d.posts>_prevNotif.posts&&_prevNotif.posts>=0){{
      showToast('COMMUNITY BOARD','New post from a member',()=>switchTab('board'));
      pushNotif('COMMUNITY','New community post','board');
    }}
    setBadge('badgeDM',d.dm);setBadge('badgeGroup',d.group);setBadge('badgePrivate',d.private);setBadge('badgeBoard',d.posts);
    document.title=d.total>0?'('+d.total+') VOX':'VOX';
...
async function loadTrafficCounter(){{
  try{{
    const d=await api('/api/traffic/public');
    if(d.ok){{
      $('tcOnline').textContent=d.online;
      $('tcToday').textContent=d.today;
      $('tcTotal').textContent=d.total;
      if($('tcMembers'))$('tcMembers').textContent=d.members;
    }}
  }}catch(e){{}}
}}
...
async function loadOnlineUsers(){{
  try{{
    ...
    if(d.ok){{
      ...
      if($('dmConvList'))loadDMConversations();
    }}
  }}catch(e){{}}
}}
...
function renderBubbles(messages,me,container){{
  if(!messages||!messages.length){{
    container.innerHTML='<div style="opacity:.3;text-align:center;margin:auto;font-size:12px;">NO MESSAGES YET</div>';
    return;
  }}
  ...
  container.innerHTML=messages.map(m=>{{
    ...
    return `<div class="bubble-row ${{mine?'mine':''}}">
      ...
      <div class="bubble-content">
        ...
        <div class="bubble-meta">${{mine?'YOU':m.sender}} &middot; ${{m.timestamp}}</div>
      </div></div>`;
  }}).join('');
...
function mobileShowChat(type){{
  if(!isMobile())return;
  const sidebarId={{dm:'dmSidebar',group:'groupSidebar',private:'privateSidebar'}}[type];
  const mainId={{dm:'dmMain',group:'groupMain',private:'privateMain'}}[type];
  const s=$(sidebarId),m=$(mainId);
...
function mobileShowSidebar(type){{
  ...
  const mainId={{dm:'dmMain',group:'groupMain',private:'privateMain'}}[type];
  const s=$(sidebarId),m=$(mainId);
  if(s)s.classList.add('mobile-show');
  if(m)m.classList.remove('mobile-show');
...
async function loadDMConversations(){{
  ...
  if(!d.ok||!d.conversations.length){{
    box.innerHTML=`<div style="padding:10px;font-size:11px;opacity:.4;">${{d.ok?'NO DMS YET':'LOGIN TO VIEW'}}</div>`;
    return;
  }}
...
async function dmDelete(u){{
  ...
  if(activeDMUser===u){{
    activeDMUser=null;
    $('dmThreadTitle').textContent='SELECT A CONVERSATION';
...
async function loadGroups(){{
  ...
  if(!d.ok||!d.groups.length){{
    box.innerHTML=`<div style="padding:10px;font-size:11px;opacity:.4;">${{d.ok?'NO CHANNELS':'LOGIN TO VIEW'}}</div>`;
    return;
  }}
...
async function createGroup(){{
  ...
  if(!name)return;
  const d=await api('/api/groups/create',{{name}});
  $('newGroupName').value='';
  if(d.ok){{loadGroups();loadGroupThread(d.id,name,false);}}
...
async function deleteGroup(gid,gname){{
  ...
  const d=await api('/api/admin/delete-channel',{{group_id:gid}});
  if(d.ok){{
    if(activeGroupId===gid){{activeGroupId=null;activeGroupName=null;$('groupThreadTitle').textContent='SELECT A CHANNEL';$('groupMessages').innerHTML='';$('groupCompose').style.display='none';}}
    loadGroups();
  }}else alert('ERROR: '+d.error);
}}
...
async function loadGroupThread(gid,gname,updateSidebar=true){{
  ...
  if(_prevNotif.groups){{delete _prevNotif.groups[String(gid)];_prevNotif.group=Object.values(_prevNotif.groups).reduce((s,v)=>s+v.count,0);}}
  ...
  if(d.admin&&d.members&&d.members.length){{
    ...
    jlBtn.onclick=()=>{{
      ...
      const dd=document.createElement('div');dd.id='memberDropdown';
      ...
      jlBtn.parentElement.style.position='relative';jlBtn.parentElement.appendChild(dd);
      setTimeout(()=>document.addEventListener('click',function h(e){{
        if(!dd.contains(e.target)&&e.target!==jlBtn){{dd.remove();document.removeEventListener('click',h);}}
      }}),0);
    }};
...
async function sendGroupMsg(){{
  ...
  $('groupInput').value='';
  await api('/api/groups/send',{{group_id:activeGroupId,content}});
...
async function loadPrivateRooms(){{
  ...
  const footer=$('privateAdminFooter');if(footer)footer.style.display=d.is_admin?'block':'none';
...
async function loadPrivateThread(rid,rname,updateSidebar=true){{
  ...
  if(_prevNotif.private_rooms){{delete _prevNotif.private_rooms[String(rid)];_prevNotif.private=Object.values(_prevNotif.private_rooms).reduce((s,v)=>s+v.count,0);}}
  setBadge('badgePrivate',_prevNotif.private);
  if(isMobile())mobileShowChat('private');
  const d=await api('/api/private/'+rid+'/messages');if(!d.ok)return;
  ...
  const mb=$('privateMembersBtn');if(mb)mb.style.display=d.is_admin?'inline-block':'none';
...
async function showPrivateMembers(){{
  const d=await api('/api/private/'+activePrivateRoomId+'/members');if(!d.ok)return;
  const existing=$('privateMemberDropdown');if(existing){{existing.remove();return;}}
  const dd=document.createElement('div');dd.id='privateMemberDropdown';
  ...
  setTimeout(()=>document.addEventListener('click',function h(e){{
    if(!dd.contains(e.target)&&e.target!==$('privateMembersBtn')){{dd.remove();document.removeEventListener('click',h);}}
  }}),0);
}}
...
async function memberSearchSuggest(){{
  ...
  if(!d.ok||!d.users.length){{box.style.display='none';return;}}
  box.style.display='block';
  box.innerHTML=d.users.map(u=>`<div style="padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--p10);"
    onmouseover="this.style.background='var(--p10)'" onmouseout="this.style.background=''"
    onmousedown="event.preventDefault();$('addMemberInput').value='${{u}}';hideMemberSuggest();">&#128100; ${{u}}</div>`).join('');
}}
...
function switchTab(tab){{
  ['DM','Group','Private','Board'].forEach(t=>{{
    const tl=t.toLowerCase();
    const tabEl=$('tab'+t),contentEl=$('tabContent'+t);
    if(tabEl)tabEl.classList.toggle('active',tl===tab);
    if(contentEl)contentEl.classList.toggle('active',tl===tab);
  }});
  if(tab==='dm'){{
    loadDMConversations();
    setBadge('badgeDM',0);
...
// ── Init ───────────────────────────────────────────────────────────────────
loadTrafficCounter();
setInterval(loadTrafficCounter,10000);
...
if($('dmConvList')){{
  ...
  setInterval(()=>{{
    if(activeDMUser)loadDMThread(activeDMUser,false);
    if(activeGroupId)loadGroupThread(activeGroupId,activeGroupName,false);
...
def home():
    ...
    def _tile(i, l, h):
        if h == "#":
            return f'<a class="tile" href="#"><i class="fas {i}"></i><div>| {l} |</div></a>'
    ...
    def _bdg(i):
        return f'<span id="{i}" style="display:none;background:var(--p);color:#000;border-radius:50%;padding:1px 5px;font-size:9px;margin-left:3px;"></span>'

    # ── Board tab ──
    board_tab = f'''<div class="tab-content" id="tabContentBoard">
    ...
    chat_tabs = (
        f'<div class="tab-bar" style="border-left:2px solid var(--p);border-right:2px solid var(--p);">'
    ...
    dm_tab = (
        '<div class="tab-content active" id="tabContentDM">'
        '<div class="comms-layout" style="border-top:none;">'
    ...
    grp_footer = (
        f'<div class="sidebar-footer" id="groupCreateFooter" style="display:{"block" if admin else "none"};">'
        ...
        f'<button class="btn-action" style="margin-top:6px;padding:5px 8px;font-size:10px;width:100%;" onclick="createGroup()">+ CREATE</button>'
        f'</div>'
    )
    grp_tab = (
        f'<div class="tab-content" id="tabContentGroup">'
        f'<div class="comms-layout" style="border-top:none;">'
        f'<div class="comms-sidebar" id="groupSidebar">'
        f'<div class="comms-sidebar-header">CHANNELS</div>'
        f'<div class="conv-list" id="groupList"><div style="padding:10px;font-size:11px;opacity:.4;">LOADING...</div></div>'
        f'{grp_footer}</div>'
        f'<div class="comms-main" id="groupMain">'
        f'<div class="comms-thread-header">'
        f'<button class="mobile-back-btn send-btn" style="padding:6px 12px;font-size:11px;margin-right:8px;display:none;" onclick="mobileShowSidebar(\'group\')">&#9664; BACK</button>'
        f'<span id="groupThreadTitle" style="flex:1;">SELECT A CHANNEL</span>'
        f'<button class="send-btn" id="joinLeaveBtn" style="display:none;font-size:10px;padding:5px 12px;"></button>'
        f'</div>'
        f'<div class="comms-messages" id="groupMessages"><div style="opacity:.3;text-align:center;margin:auto;font-size:12px;">SELECT A CHANNEL</div></div>'
        f'<div class="comms-compose" id="groupCompose" style="display:none;">'
        f'<input type="text" id="groupInput" placeholder="BROADCAST..." onkeydown="if(event.key===\'Enter\')sendGroupMsg()">'
        f'<button class="send-btn" onclick="sendGroupMsg()">&#9658;</button>'
        f'</div></div></div></div>'
    )

    prv_tab = (
        '<div class="tab-content" id="tabContentPrivate">'
        '<div class="comms-layout" style="border-top:none;">'
    ...
    chat_panel = cyber_box(
        "// CHAT //",
        dm_tab + grp_tab + prv_tab + board_tab,
        ...
        extra_header=chat_tabs,
        body_style="border-top:none;",
        border_top=False
    ) if user else ""

    search_bar = (
        '<div class="search-box" style="width:100%;margin:0 0 24px;">'
    ...
    install_banner = (
        '<div id="installBanner" style="display:block;width:100%;margin:0 0 16px;box-sizing:border-box;">'
        ...
        '</div></div>'
        '<div id="iosInstallMsg" style="display:none;border:1px solid var(--p30);border-top:none;border-radius:0 0 var(--r) var(--r);padding:8px 16px;font-size:10px;opacity:.7;letter-spacing:1px;">&#63743; ON IOS: TAP THE SHARE BUTTON THEN &ldquo;ADD TO HOME SCREEN&rdquo;</div>'
        '</div>'
        '<script>'
    ...
    news_panel = cyber_box(
        "// LIVE NEWS //",
        '<div id="newsFeed" style="padding:16px;opacity:.4;text-align:center;font-size:11px;">&#128256; FETCHING NEWS...</div>',
        title_right='<span id="newsFeedStatus" style="font-size:9px;opacity:.4;letter-spacing:1px;">LOADING...</span>',
        extra_header=news_tabs,
        max_h="320px",
        border_top=False
    ) if user else ""

    content = (
        f'<div class="command-wrapper">'
        f'<div style="text-align:center;width:100%;"><div class="tile-grid">{tiles}</div></div>'
        ...
        f'<div class="three-column-grid">'
        f'<div class="column"><h3>THE TRUTH</h3><p>Wealth gap and corporate reliance truth.</p><a class="btn-action" href="https://www.youtube.com/watch?v=pb0OCI9qwIU" target="_blank" rel="noopener noreferrer">&#9658; WATCH</a></div>'
        ...
        f'<div class="column"><h3>COMMUNITY</h3><p>Join our TikTok community and say hello!</p><a class="btn-action" href="#" target="_blank" rel="noopener noreferrer">&#9658; ACCESS</a></div>'
        f'</div></div>'
    )
...
def api_register():
    d = request.json or {}
    u, p, t = d.get("username", "").strip(), d.get("password", ""), d.get("theme", "green")
    if not u or not p:
        return err("FIELDS REQUIRED")
    if len(u) < 3:
        return err("USERNAME TOO SHORT (MIN 3)")
    if len(p) < 6:
    ...
    try:
        with db() as con:
            execute(con, "INSERT INTO users(username,password_hash,theme,is_admin) VALUES(%s,%s,%s,%s)",
                    (u, hash_pw(p), t, 1 if u == ADMIN_USER else 0))
            group_ids = [r[0] for r in fetchall(con, "SELECT id FROM groups")]
...
def api_theme():
    ...
    with db() as con:
        execute(con, "UPDATE users SET theme=%s WHERE username=%s", (t, me()))
    session["theme"] = t
    return ok()
...
def api_user_search():
    ...
    if not q:
        return ok(users=[])
    with db() as con:
        rows = fetchall(con, "SELECT username FROM users WHERE username ILIKE %s LIMIT 10", (f"%{q}%",))
...
def api_posts():
    ...
    for pid, rxu, emoji in rxrows:
        ...
        if rxu == u:
            rx[pid][emoji]["mine"] = True
    posts = [
        {"id": r[0], "username": r[1], "content": r[2], "created_at": str(r[3])[:16],
         "reactions": rx.get(r[0], {}), "can_delete": is_admin(u) or r[1] == u}
        for r in rows
    ]
...
def api_posts_create():
    ...
    if len(content) > 500:
        return err("TOO LONG")
    with db() as con:
        execute(con, "INSERT INTO posts(username,content) VALUES(%s,%s)", (me(), content))
    return ok()
...
def api_posts_react():
    ...
    if not post_id or emoji not in VALID_EMOJIS:
        return err("INVALID")
    u = me()
    with db() as con:
        ...
        if existing:
            if existing[0] == emoji:
                execute(con, "DELETE FROM post_reactions WHERE post_id=%s AND username=%s", (post_id, u))
            else:
                execute(con, "UPDATE post_reactions SET emoji=%s WHERE post_id=%s AND username=%s", (emoji, post_id, u))
        else:
            execute(con, "INSERT INTO post_reactions(post_id,username,emoji) VALUES(%s,%s,%s)", (post_id, u, emoji))
    return ok()
...
def api_admin_users():
    ...
    with db() as con:
        rows = fetchall(con, "SELECT username,is_admin,created_at FROM users ORDER BY is_admin DESC,created_at ASC")
...
def api_admin_set():
    ...
    if target == ADMIN_USER:
        return err("CANNOT MODIFY ROOT ADMIN")
    with db() as con:
        execute(con, "UPDATE users SET is_admin=%s WHERE username=%s", (1 if grant else 0, target))
    return ok()
...
def api_admin_remove_user():
    ...
    if target == ADMIN_USER:
        return err("CANNOT REMOVE ROOT ADMIN")
    with db() as con:
        for tbl, col in [("messages", "sender"), ("messages", "recipient"),
                         ("group_messages", "sender"), ("group_members", "username")]:
...
def api_admin_dm_log():
    if e := require_admin(): return e
    with db() as con:
        rows = fetchall(con, "SELECT id,sender,recipient,content_enc,timestamp FROM messages ORDER BY timestamp DESC LIMIT 200")
...
def api_admin_delete_dm():
    if e := require_admin(): return e
    with db() as con:
        execute(con, "DELETE FROM messages WHERE id=%s", (request.json.get("id"),))
    return ok()
...
def api_admin_user_chat():
    ...
    with db() as con:
        if not fetchone(con, "SELECT id FROM users WHERE username=%s", (username,)):
            return err("USER NOT FOUND")
        rows = fetchall(con,
            "SELECT id,sender,recipient,content_enc,timestamp FROM messages WHERE sender=%s OR recipient=%s ORDER BY timestamp ASC",
            (username, username))
    convos = {}
    for r in rows:
...
def api_admin_delete_convo():
    ...
    with db() as con:
        execute(con, "DELETE FROM messages WHERE (sender=%s AND recipient=%s) OR (sender=%s AND recipient=%s)", (u1, u2, u2, u1))
    return ok()
...
def api_admin_delete_channel():
    ...
    if not gid:
        return err("MISSING GROUP ID")
    with db() as con:
        execute(con, "DELETE FROM group_messages WHERE group_id=%s", (gid,))
        execute(con, "DELETE FROM group_members WHERE group_id=%s", (gid,))
        execute(con, "DELETE FROM group_banned WHERE group_id=%s", (gid,))
        execute(con, "DELETE FROM groups WHERE id=%s", (gid,))
    return ok()
...
def api_admin_delete_group_msg():
    if e := require_admin(): return e
    with db() as con:
        execute(con, "DELETE FROM group_messages WHERE id=%s", (request.json.get("id"),))
    return ok()
...
def api_admin_lock_channel():
    ...
    d = request.json or {}
    with db() as con:
        execute(con, "UPDATE groups SET locked=%s WHERE id=%s", (1 if d.get("lock") else 0, d.get("group_id")))
    return ok()
...
def api_admin_reset_requests():
    if e := require_admin(): return e
    with db() as con:
        rows = fetchall(con, "SELECT id,username,temp_password,status,requested_at FROM password_resets WHERE status='pending' ORDER BY requested_at DESC")
...
def api_admin_reset_approve():
    ...
    if not rid or not temp_pw:
        return err("MISSING FIELDS")
    if len(temp_pw) < 4:
        return err("TEMP PASSWORD TOO SHORT")
    with db() as con:
...
def api_admin_reset_deny():
    ...
    with db() as con:
        execute(con, "UPDATE password_resets SET status='denied' WHERE id=%s", (rid,))
    return ok()
...
def track_visit():
    ...
    now = utc_now()
    with db() as con:
        execute(con, "INSERT INTO visits(date,ip) VALUES(%s,%s) ON CONFLICT DO NOTHING",
                (datetime.date.today().isoformat(), get_ip()))
        u = session.get("username")
        if u:
            execute(con, "INSERT INTO user_sessions(username,last_seen) VALUES(%s,%s) ON CONFLICT (username) DO UPDATE SET last_seen=EXCLUDED.last_seen",
                    (u, now))
...
def api_traffic_public():
    ...
    with db() as con:
        ...
        if u:
            execute(con, "INSERT INTO user_sessions(username,last_seen) VALUES(%s,%s) ON CONFLICT (username) DO UPDATE SET last_seen=EXCLUDED.last_seen", (u, now))
...
def api_online():
    ...
    cutoff = utc_cutoff(2)
    with db() as con:
        rows = fetchall(con, "SELECT username FROM user_sessions WHERE last_seen >= %s", (cutoff,))
            ...
            for item in blocks[:8]:
                ...
                for pat in [r'<link[^>]+href="([^"]+)"', r'<link[^>]*>(https?://[^<]+)</link>', r'<guid[^>]*>(https?://[^<]+)</guid>']:
                    ...
                    if m:
                        url = m.group(1).strip()
                        break
                # Extract publish date for timestamp display
                ...
                if title and url and title not in seen:
                    ...
                    items.append({
                        "title": title[:120],
                        "url": url,
                        "desc": get_tag(item, 'description')[:200] or get_tag(item, 'summary')[:200],
                        "cat": category,
                        "pub_date": pub_date,
                    })
        except Exception:
...
def api_dm_conversations():
    ...
    with db() as con:
        ...
        partners = fetchall(con,
            "SELECT CASE WHEN sender=%s THEN recipient ELSE sender END as partner,MAX(timestamp) "
            "FROM messages WHERE sender=%s OR recipient=%s GROUP BY partner ORDER BY 2 DESC",
            (u, u, u))
        convos = []
        for (partner, _) in partners:
            cnt = unread_count(con, 'messages', 'sender', partner, u, read_dm_at.get(partner, '1970-01-01'))
            # unread_count checks sender=partner, recipient=u via the generic helper — need custom query here
            row = fetchone(con, "SELECT COUNT(*) FROM messages WHERE sender=%s AND recipient=%s AND timestamp>%s",
                           (partner, u, read_dm_at.get(partner, '1970-01-01')))
...
def api_dm_thread():
    ...
    with db() as con:
        rows = fetchall(con,
            ...
            "WHERE (sender=%s AND recipient=%s) OR (sender=%s AND recipient=%s) ORDER BY timestamp ASC LIMIT 100",
            (u, other, other, u))
...
def api_dm_send():
    ...
    u = me()
    with db() as con:
        if not fetchone(con, "SELECT id FROM users WHERE username=%s", (to,)):
            return err("USER NOT FOUND")
        if fetchone(con, "SELECT 1 FROM dm_blocked WHERE (blocker=%s AND blocked=%s) OR (blocker=%s AND blocked=%s)", (u, to, to, u)):
            return err("CANNOT MESSAGE THIS USER")
...
def api_dm_unblock():
    ...
    other = (request.json or {}).get("username", "").strip()
    with db() as con:
        execute(con, "DELETE FROM dm_blocked WHERE blocker=%s AND blocked=%s", (me(), other))
    return ok()
...
def api_groups():
    ...
    return ok(groups=[
        {"id": r[0], "name": r[1], "member": r[0] in member_ids, "locked": bool(r[2]),
         "banned": r[0] in banned_ids, "unread": unread.get(r[0], 0)}
        for r in rows
    ])
...
def api_group_create():
    ...
    try:
        with db() as con:
            row = fetchone(con, "INSERT INTO groups(name,created_by) VALUES(%s,%s) RETURNING id", (name, me()))
            gid = row[0]
            users = [r[0] for r in fetchall(con, "SELECT username FROM users")]
            for uname in users:
...
def api_group_send():
    ...
    u = me()
    with db() as con:
        if not fetchone(con, "SELECT 1 FROM group_members WHERE group_id=%s AND username=%s", (gid, u)):
...
def api_group_kick():
    if e := require_admin(): return e
    d = request.json or {}
    gid, target = d.get("group_id"), d.get("username", "").strip()
...
def api_group_join():
    if not logged_in():
        return jsonify({"ok": False})
    with db() as con:
        execute(con, "INSERT INTO group_members(group_id,username) VALUES(%s,%s) ON CONFLICT DO NOTHING",
...
def api_group_leave():
    if not logged_in():
        return jsonify({"ok": False})
    with db() as con:
        execute(con, "DELETE FROM group_members WHERE group_id=%s AND username=%s",
                (request.json.get("group_id"), me()))
    return ok()
...
def api_group_rename():
    if e := require_admin(): return e
    d = request.json or {}
    gid, name = d.get("id"), d.get("name", "").strip().upper()
    if not gid or not name:
        return err("MISSING FIELDS")
    try:
        with db() as con:
            execute(con, "UPDATE groups SET name=%s WHERE id=%s", (name, gid))
        return ok()
    except Exception:
...
def api_private_rooms():
    ...
    with db() as con:
        if admin:
            rows = fetchall(con, "SELECT id,name FROM private_rooms ORDER BY name ASC")
        else:
            rows = fetchall(con,
                "SELECT r.id,r.name FROM private_rooms r JOIN private_room_members m ON r.id=m.room_id WHERE m.username=%s ORDER BY r.name ASC",
                (u,))
...
def api_private_create():
    ...
    with db() as con:
        row = fetchone(con, "INSERT INTO private_rooms(name,created_by) VALUES(%s,%s) RETURNING id", (name, me()))
        rid = row[0]
...
def api_private_messages(rid):
    ...
    with db() as con:
        if not admin:
            if not fetchone(con, "SELECT 1 FROM private_room_members WHERE room_id=%s AND username=%s", (rid, u)):
...
def api_private_send():
    if e := require_login(): return e
    d = request.json or {}
    rid, content = d.get("room_id"), d.get("content", "").strip()
...
def api_private_members(rid):
    if e := require_admin(): return e
    with db() as con:
        rows = fetchall(con, "SELECT username FROM private_room_members WHERE room_id=%s ORDER BY username", (rid,))
...
def api_private_add_member():
    if e := require_admin(): return e
    d = request.json or {}
    rid, username = d.get("room_id"), d.get("username", "").strip()
...
def api_private_remove_member():
    ...
    if not rid or not username:
        return err("MISSING FIELDS")
    with db() as con:
        execute(con, "DELETE FROM private_room_members WHERE room_id=%s AND username=%s", (rid, username))
    return ok()
...
def api_private_rename():
    if e := require_admin(): return e
    d = request.json or {}
    rid, name = d.get("id"), d.get("name", "").strip().upper()
    if not rid or not name:
        return err("MISSING FIELDS")
    with db() as con:
        execute(con, "UPDATE private_rooms SET name=%s WHERE id=%s", (name, rid))
    return ok()
...
def api_notifications():
    ...
    with db() as con:
        ...
        dm_unread = sum(
            fetchone(con, "SELECT COUNT(*) FROM messages WHERE sender=%s AND recipient=%s AND timestamp>%s",
                     (s, u, read_dm_at.get(s, '1970-01-01')))[0]
            for s in senders
        )
        ...
        if is_admin(u):
            priv_rows = fetchall(con, "SELECT id,name FROM private_rooms ORDER BY id")
        else:
            priv_rows = fetchall(con,
                "SELECT r.id,r.name FROM private_rooms r JOIN private_room_members m ON r.id=m.room_id WHERE m.username=%s ORDER BY r.id",
                (u,))
        ...
        new_posts = fetchone(con, "SELECT COUNT(*) FROM posts WHERE username!=%s AND created_at>%s",
                             (u, posts_read[0] if posts_read else '1970-01-01'))[0]

    group_total = sum(v["count"] for v in groups_unread.values())
    priv_total = sum(v["count"] for v in privrooms_unread.values())
    return ok(
        dm=dm_unread, groups=groups_unread, group=group_total,
        private_rooms=privrooms_unread, private=priv_total,
        posts=new_posts, total=dm_unread + group_total + priv_total + new_posts
    )
...
def api_mark_read():
    ...
    with db() as con:
        execute(con,
            "INSERT INTO chat_read_at(username,chat_type,chat_id,read_at) VALUES(%s,%s,%s,%s) "
            "ON CONFLICT (username,chat_type,chat_id) DO UPDATE SET read_at=EXCLUDED.read_at",
            (me(), chat_type, chat_id, now))
        if chat_type == "dm":
            execute(con, "UPDATE messages SET read=1 WHERE recipient=%s AND sender=%s", (me(), chat_id))
    return ok()
...
def api_reset_request():
    ...
    with db() as con:
        ...
        if fetchone(con, "SELECT id FROM password_resets WHERE username=%s AND status='pending'", (username,)):
            return ok()
...
def api_push_unsubscribe():
    ...
    if endpoint:
        with db() as con:
            execute(con, "DELETE FROM push_subscriptions WHERE username=%s AND endpoint=%s", (me(), endpoint))
    return ok()
...
def api_ask():
    ...
    try:
        payload = _json.dumps({
            "contents": [{"parts": [{"text": "Answer concisely under 150 words. Plain text only, no markdown.\n\n" + query}]}],
...
def manifest():
    data = {
        "name": "Vox Populi", "short_name": "VOX", "description": "Vox Populi Community",
        "start_url": "/", "display": "standalone", "background_color": "#000000",
        "theme_color": "#00ff00", "orientation": "portrait-primary",
        "icons": [
...
def _svg_icon(size, text_y, font_size, sub_y=None, sub_text=None):
    ...
    sub = (f'<text x="{size//2}" y="{sub_y}" text-anchor="middle" font-family="monospace" font-size="{font_size//4}" fill="#00ff00" opacity="0.6" letter-spacing="6">{sub_text}</text>'
           if sub_text else '')
    r = size // 2
    cr = int(r * 0.85)
...
def emergency_reset():
    new_pw = "Vox2024!"
    with db() as con:
        execute(con, "UPDATE users SET password_hash=%s WHERE username=%s", (hash_pw(new_pw), ADMIN_USER))
