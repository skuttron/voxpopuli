...
# FIX A: Add Secure flag so cookies survive HTTPS reverse-proxy (Render/Railway/Heroku)
app.config['SESSION_COOKIE_SECURE']=True
# FIX B: Trust the X-Forwarded-Proto header from the proxy so Flask knows it's HTTPS
...
def shell(content,user=None,theme="green",unread=0):
    ...
    # FIX C: All JS fixes consolidated - session cookie, mark-read guard, tab init
    JS=f"""
...
async function doLogin(){{
  ...
  if(!username||!password){{errEl.textContent='FIELDS REQUIRED';return;}}
  try{{
...
function switchTab(tab){{
  ['DM','Group','Private','Board'].forEach(t=>{{
    ...
    if(contentEl){{contentEl.style.display=tl===tab?'block':'none';contentEl.classList.toggle('active',tl===tab);}}
  }});
  // FIX E: Only call mark-read with a real id, never an empty string
  if(tab==='dm'){{loadDMConversations();setBadge('badgeDM',0);if(activeDMUser)api('/api/mark-read',{{type:'dm',id:activeDMUser}});_prevNotif.dm=0;}}
  else if(tab==='group'){{loadGroups();}}
  else if(tab==='private'){{loadPrivateRooms();}}
  else if(tab==='board'){{loadPosts();markPostsRead();}}
}}
...
if($('dmConvList')){{
  ['tabContentDM','tabContentGroup','tabContentPrivate','tabContentBoard'].forEach((id,i)=>{{
    const el=$(id);if(el)el.style.display=i===0?'block':'none';
  }});
