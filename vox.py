...
async function loadNewsFeed(){{
  try{{
    ...
    function getPriority(item){{const t=(item.title+' '+(item.desc||'')).toLowerCase();if(redWords.some(w=>t.includes(w)))return '#f44';if(yelWords.some(w=>t.includes(w)))return '#fb0';return srcColors[item.cat]||'var(--p)';}}
    const priorityLabel={{'#f44':'&#9888; ALERT','#fb0':'&#9650; WATCH','#00ff00':'&#9679; CLEAR'}};
    feed.innerHTML=d.items.map(item=>{{const c=getPriority(item),pl=priorityLabel[c]||'';return `<div style="padding:10px 14px;border-bottom:1px solid var(--p10);border-left:3px solid ${{c}};display:flex;flex-direction:column;gap:4px;"><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;"><span style="font-size:9px;padding:2px 6px;border:1px solid ${{c}};color:${{c}};border-radius:4px;flex-shrink:0;font-weight:bold;">${{pl}} ${{item.cat}}</span><a href="${{item.url}}" target="_blank" rel="noopener noreferrer" style="color:var(--p);font-size:12px;font-weight:bold;text-decoration:none;line-height:1.4;">${{item.title}}</a></div>${{item.desc?`<div style="font-size:11px;opacity:.6;line-height:1.5;text-transform:none;padding-left:2px;">${{item.desc}}</div>`:''}}</div>`;
    }}).join('');
...
def api_register():
    ...
    try:
        with db() as con:
            cur=con.cursor()
            cur.execute("INSERT INTO users(username,password_hash,theme,is_admin) VALUES(%s,%s,%s,%s)",(u,hash_pw(p),t,1 if u==ADMIN_USER else 0))
            cur.execute("SELECT id FROM groups")
...
def api_admin_user_chat():
    ...
    with db() as con:
        ...
        cur.execute("SELECT id,sender,recipient,content_enc,timestamp FROM messages WHERE sender=%s OR recipient=%s ORDER BY timestamp ASC",(username,username))
        rows=cur.fetchall()
    convos={}
    for r in rows:
...
def api_dm_conversations():
    ...
    with db() as con:
        ...
        cur=con.cursor()
        cur.execute("SELECT CASE WHEN sender=%s THEN recipient ELSE sender END as partner,MAX(timestamp) FROM messages WHERE sender=%s OR recipient=%s GROUP BY partner ORDER BY 2 DESC",(u,u,u))
...
def api_dm_thread():
    ...
    with db() as con:
        cur=con.cursor()
        cur.execute("SELECT sender,content_enc,timestamp FROM messages WHERE (sender=%s AND recipient=%s) OR (sender=%s AND recipient=%s) ORDER BY timestamp ASC LIMIT 100",(u,other,other,u))
        rows=cur.fetchall()
...
def api_private_rooms():
    ...
    with db() as con:
        cur=con.cursor()
        if admin: cur.execute("SELECT id,name FROM private_rooms ORDER BY name ASC")
        else: cur.execute("SELECT r.id,r.name FROM private_rooms r JOIN private_room_members m ON r.id=m.room_id WHERE m.username=%s ORDER BY r.name ASC",(u,))
        rows=cur.fetchall()
...
def api_notifications():
    ...
    with db() as con:
        ...
        if is_admin(u): cur.execute("SELECT id,name FROM private_rooms ORDER BY id")
        else: cur.execute("SELECT r.id,r.name FROM private_rooms r JOIN private_room_members m ON r.id=m.room_id WHERE m.username=%s ORDER BY r.id",(u,))
