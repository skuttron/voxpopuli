...
NAV_ITEMS = [
    ...
    ("fa-box-open",        "STOCK UP", "#"),
    ("fa-map-marked-alt",  "MAPS",     "#"),
    ("fa-archive",         "CANNING",  "#"),
...
.scanline-a{{position:fixed;left:0;width:100%;height:10px;background:linear-gradient(to bottom,transparent,var(--p30),transparent);filter:blur(2px);animation:scan 5s linear infinite 1s;pointer-events:none;z-index:1}}
.scanline-b{{position:fixed;left:0;width:100%;height:4px;background:var(--p);opacity:.12;animation:scan 9s linear infinite 4s;pointer-events:none;z-index:1}}
.scanline-c{{position:fixed;left:0;width:100%;height:24px;background:linear-gradient(to bottom,transparent,{p}22,transparent);filter:blur(5px);animation:scan 18s linear infinite 0s;pointer-events:none;z-index:1}}
...
async function loadNewsFeed(){{
  try{{
    ...
    if(!feed)return;
    if(!d.ok||!d.items||!d.items.length){{feed.innerHTML='<div style="padding:12px;opacity:.4;text-align:center;font-size:11px;">NO FEED AVAILABLE</div>';return}}
...
function markPostsRead(){{
  ...
  _prevNotif.posts=0;
  const b=$('badgePosts');if(b){{b.textContent='';b.style.display='none';}}
}}
...
async function submitPost(){{
  const content=$('postContent').value.trim(),errEl=$('postErr');errEl.textContent='';
  ...
  $('postContent').value='';updatePostCount($('postContent'));loadPosts();
}}
...
function requestNotifPermission(){{
  ...
  if(Notification.permission!=='denied')Notification.requestPermission().then(p=>{{_notifPermission=p==='granted';if(_notifPermission)setupPushSubscription();}});
}}
...
async function checkNotifications(){{
  try{{
    ...
    if(!_notifReady){{
      ...
      document.title=d.total>0?'('+d.total+') VOX':'VOX';return;
    }}
...
async function loadDMThread(username,updateSidebar=true){{
  ...
  api('/api/mark-read',{{type:'dm',id:username}});_prevNotif.dm=0;
  const d=await api('/api/dm/thread?with='+encodeURIComponent(username));if(!d.ok)return;
...
async function createPrivateRoom(){{
  ...
  const d=await api('/api/private/create',{{name}});$('newRoomName').value='';
  if(d.ok){{loadPrivateRooms();loadPrivateThread(d.id,name);}}else alert('ERROR: '+d.error);
}}
...
async function showPrivateMembers(){{
  ...
  $('privateMembersBtn').parentElement.style.position='relative';$('privateMembersBtn').parentElement.appendChild(dd);
...
async function addPrivateMember(){{
  hideMemberSuggest();const u=$('addMemberInput').value.trim();if(!u)return;
  const d=await api('/api/private/add-member',{{room_id:activePrivateRoomId,username:u}});
  if(!d.ok){{alert('ERROR: '+d.error);return}}
  const dd=$('privateMemberDropdown');if(dd)dd.remove();showPrivateMembers();
}}
...
def api_news():
    ...
    def fetch(url):
        req = urllib.request.Request(url, headers={
            "User-Agent": random.choice(UAS), "Accept": "application/rss+xml,application/xml,text/xml,*/*",
        ...
        try:
            ...
            for item in blocks[:8]:
                ...
                if title and url and title not in seen:
                    seen.add(title)
                    items.append({"title": title[:120], "url": url, "desc": tag(item,'description')[:200] or tag(item,'summary')[:200],
                                  "date": (tag(item,'pubDate') or tag(item,'published') or tag(item,'updated'))[:30], "cat": category})
        except: continue
...
def api_ask():
    ...
    try:
        payload = _json.dumps({"contents": [{"parts": [{"text": "Answer concisely under 150 words. Plain text only, no markdown.\n\n" + query}]}],
                                "generationConfig": {"maxOutputTokens": 300, "temperature": 0.7}}).encode()
